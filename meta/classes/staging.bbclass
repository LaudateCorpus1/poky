...
#
# Target build output, stored in do_populate_sysroot or do_package can depend not only
# upon direct dependencies but also indirect ones. A good example is linux-libc-headers.
# The toolchain depends on this but most target recipes do not. There are some headers
# which are not used by the toolchain build and hence can change without changing the
# sysroot output of that recipe yet they can influence others.
#
